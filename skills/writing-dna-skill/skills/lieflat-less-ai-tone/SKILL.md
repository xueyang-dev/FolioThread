...
**单独使用**：用户直接要求清理某段文字的 AI 痕迹时，本 skill 可以独立执行，不需要蒸馏产物，也不需要用户提供语料。这是最常见的用法。

**配合写作蒸馏器**：作为写作流程的收尾工序——先蒸馏出 Writing-DNA，按 DNA 写作，写完后由用户决定是否用本 skill 清理。以蒸馏为目的时，本步骤是可选项，不是必经环节。
