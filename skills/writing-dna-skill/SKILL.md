...
description: 从至少 20 篇完整文章中蒸馏可复用的写作 DNA，分析语言、文章结构、选题逻辑、素材策略、认知框架和视觉风格，并生成 Writing-DNA.md；按该 DNA 写作时会读取全部蒸馏产物和 5 篇相关原文。用于中英文作者、账号、品牌或出版物的风格分析与一致性写作。内置的去 AI 味规则集也可单独调用，用于清理成稿中的 AI 痕迹。 Distill reusable Writing DNA from at least 20 complete articles for Chinese or English authors, publications, brands, and accounts; use for language, structure, topic logic, source strategy, cognitive-frame, and visual-style analysis. Includes a standalone rule set for removing AI writing tells.
---
...
**默认入口是蒸馏**：用户给语料、要分析某个作者的风格、要复刻某种写法时，都走第三节的 Step 1-7 蒸馏流程，蒸馏完成后按第六节使用产物写作。

**去 AI 味是独立入口**：`skills/lieflat-less-ai-tone/` 是一个可以单独使用的 skill。用户直接说"帮我去掉这篇的 AI 味 / 这段读起来太像 AI / 清理一下 AI 痕迹"，且没有蒸馏需求时，直接读取 `skills/lieflat-less-ai-tone/SKILL.md` 并执行，不要先做蒸馏，也不要要求用户提供语料。

**以蒸馏为目的时，去 AI 味只是写作完成后的可选项**，不是必经步骤。按第 6.3 节处理。
...
写作完成后，**主动询问用户是否需要去掉 AI 痕迹**，例如："要不要再用去 AI 味的规则清理一遍？"

- 用户同意时，读取 `skills/lieflat-less-ai-tone/SKILL.md` 并按其规则执行
- 用户不需要或没有回应时，交付当前稿件即可，不要自行执行
- 去 AI 味只处理其规则清单内的问题，不改文章框架，也不覆盖本次写作已遵循的 DNA 特征

如果用户一开始就只想去 AI 味、没有蒸馏需求，直接走去 AI 味 skill，不必进入本节流程。
