...
本文件记录 Folith / 译页的用户可见变更。历史条目中的 FolioThread、TransPraxis / 译践 是此前产品名；`v0.4.0` 已正式发布。

## [0.4.0] - 2026-09-22

> **边界说明**：本节记录 Folith / 译页 v0.4.0 的发布内容，
...
- OpenAI 兼容中转站返回 `INSUFFICIENT_BALANCE`、无效 API Key、限流、拒绝访问或服务不可用时，
  连接测试、模型目录获取、快速画像、定点翻译和任务运行状态会显示对应的中文处理提示；
  例如余额不足会提示「余额不足，请充值或更换 API Key」，不再只暴露 HTTP 错误码。
- 技术日志仍保留诊断所需的原始异常，产品界面不会回显 SDK 原始错误体或密钥内容。

### 修复：译文保存闭环与防丢稿（发布阻断）

这一组是"新人能不能安全地改完并保存"的问题。共同点是**界面看起来正常，稿子却在丢**。

- **段落编辑不再包在表单里**。此前译文输入框放在 `st.form` 中，而"是否显示保存按钮"
  由后端根据"内容是否已修改"决定 —— 表单输入在提交前根本不到达后端，于是形成死循环：
  必须先提交才知道要不要显示提交按钮。现在去掉表单，编辑内容通过 `on_change` 即时写入
  **草稿层**（`translation_edit_drafts`，按 `job_id|segment_id` 键），保存入口基于草稿
  真实渲染；当前段落常驻保存入口，其余段落仅在真正改过时出现。
- **切换不丢，刷新也不丢**（本轮补齐）。切段落、切筛选、切任务都不丢未保存内容（草稿层
  与渲染窗口无关）；刷新页面 / 关标签页现在也不丢 —— 草稿落盘在任务目录的
  `translation_drafts.json`，新会话会把它恢复回输入框并**明确告知**"已从本机任务目录恢复"。
  草稿**不是正式译文**：它不进 `state.json`、不参与交付资产、不会继承或改变审校结论。
  横幅同时提供"保存全部 / 丢弃全部"。此前只有"切换不丢"这一个承诺，刷新静默丢稿。
- **交付入口统一处理未保存修改**（本轮补齐）。冻结交付只定格**已保存**的译文，而草稿
  不在文档里 —— 有"未保存"横幅不等于交付入口知道这件事。现在冻结前检查当前任务草稿，
  有未保存就阻止冻结并给出"保存这 N 处并继续 / 返回编辑"，避免用户导出修改前的版本。
- **覆盖冲突提示**（本轮补齐）。保存时若磁盘上的译文已被别处改过（另一个标签页、
  后台重译、批量操作），不静默覆盖，而是摆出"你的草稿 / 磁盘上的当前译文"让用户选：
  用我的覆盖、采用磁盘版本、稍后处理。参照系是**草稿自己的 baseline**，不是本次渲染的
  `pair.target` —— 后者在后台写回之后已经是新值，拿它比永远比出"不冲突"。
- **草稿必须回填输入框**（本轮修出的真实缺陷）。段落被筛掉或滚出渲染窗口时，Streamlit
  会丢弃该 keyed widget 的值；此前只回填已保存的译文，于是用户切回来看到的是**旧译文**，
  点"保存"把旧值写回文档、同时把草稿当成已保存丢弃 —— 一次点击同时丢掉用户输入和
  磁盘上的正确内容，而横幅还显示"有未保存修改"。现在有草稿时以草稿为输入框初值。
- **服务端换内容时必须换 key**（本轮浏览器实测修出的真实缺陷）。`st.text_area` 的 element id
  只由 `(user_key, max_chars)` 决定，默认值不参与，所以"清掉 session_state 里的 key 让
  widget 用新默认值重建"这条常见做法**对已经被敲过字的输入框无效**：前端组件不重挂载，
  它保留自己的输入。表现是点了「复制原文到译文」/「采用磁盘上的版本」/「丢弃未保存的
  修改」/「丢弃全部」之后，框里还是自己刚才写的字。现在这些动作统一走
  `_reset_translation_editor`（换 key 强制重挂载）。**`AppTest` 抓不到它** —— 清 key 之后
  AppTest 下一轮就用新默认值重建，断言照样通过；只有真实浏览器能证伪。
- **保存反馈具体到段落**，并区分保存与确认：「已保存 · 第 N 段」与审校确认是两回事。

### 新增：保存并继续 + 连续编辑动作

- **保存并进入下一段 / 下一未确认段**：形成连续审校路径。保存与"确认/审校"语义分开：
  保存**不会**把段落标记为已审校；若这一段此前已审校，人工修改后那次审校结论会作废
  并需要重新审校（`core.save_translation_edit` 会清 `reviewed` / `review_status` 并重算计数）。
- **复制原文到译文**：把当前段原文填进译文草稿（不落盘），适合专名、代码、译者新增内容。
- **翻译/重译当前段落**：在当前段落直接可达（不必回到工具栏）；无可用引擎时禁用并说明原因。

### 修复：拆分 / 合并 / 插入后的数据对应与撤销

- **稳定段落身份 `segment_uid`**：此前段落按**位置索引**贯穿编辑框、选中态、问题定位和
  候选内容，任何结构操作都会让"旧位置的新段落"继承旧编辑值。现在每段有稳定身份
  （初始与交付资产 `segment_id` 一致，避免升级漂移；结构操作新建的段落用单调递增、
  **永不复用**的新身份），编辑缓存按身份清理而非按位置。
- **拆分新半段的身份**：split 分支此前漏传 `job_id`，新半段的身份退化成序号（如 `u000003`），
  丢掉了任务前缀。已修正。
- **合并确认框移出表单**。此前确认勾选框在表单内、提交按钮又按后端值禁用，形成同一个
  死循环的第二个实例："勾选后按钮无法解锁"。现在勾选与按钮同轮生效。
- **撤销最近一次结构操作**：拆分 / 合并 / 插入 / 排除 / 恢复都可撤销，恢复的是
  **内容快照**（`undo_index` + 原文/译文对该），不只是操作名称和位置。
- **排除 / 恢复会改变后续每一段的索引含义**，因此过期判定从"只判被操作那一段"改为
  **整表判过期**；同理重算审校统计。
- **恢复的段落不得继承旧审校标记**：`include` 会把 `reviewed` / `review_status` /
  `accepted_target` 清回未审校 —— 一个刚插回原文的段落不该点亮旧审校结论。

### 新增：排除无效段落

导入扫描件后常见 OCR 垃圾行、页码、重复页眉。这类内容**不能靠改字符**解决，此前也没有
通用入口（原文不能存空、只能删手动插入且仍为空的段落）。

- **排除本段**：保留原文、随时可恢复；被排除内容不进入翻译、不计入待完成数量、
  不参与交付检查。排除记录落在 `excluded_segments`，恢复位置由原文锚点反推
  （原文在两段之间找到就插回原位），因此排除/恢复不依赖瞬时索引。
- **导出规则明确**：排除段落的数量与清单写入交付包（`excluded_segments.md` /
  `excluded_segments.json`），`bilingual.jsonl` 不含被排除的原文。

### 修复：Agent 候选分类与失败处理（发布阻断）

- **动作名分类不再中英错位**。改写按钮此前传中文动作名，而候选分类按英文内部名判断，
  "改写"可能被归为只读诊断 —— 于是候选没有"应用到译文"，用户以为改写了其实没有。
  现在分类按内部名判断，界面文案与内部名解耦。
- **成功 / 空 / 失败分开表示**。模型调用失败此前返回普通错误字符串；勾选"作为改写候选"
  时**错误文字本身会成为可应用内容**。现在三者结构不同：失败永不进入候选、永不写回译文，
  空结果明确说明"模型没有返回内容"，诊断结果不可应用。

### 新增：导入范围提示（不静默漏内容）

- 此前 DOCX 提取只遍历正文段落，**表格等内容静默不进翻译**。现在提取时检测并统计
  未纳入的部件（表格、页眉/页脚、文本框、图片、公式、脚注/尾注、批注），在任务里
  以报告形式展示：**提取了什么、哪些没有纳入、导出是重建译文文档而非原格式保真**。
- PDF / OCR 路径同样给出事实（是否为扫描件、是否走了 OCR），报告版本化为
  `EXTRACTION_REPORT_VERSION`，坏文件按 best-effort 出报告而不是静默当作空文档。
- 原格式保真（DOCX Contract v1）仍按蓝图留到发布后，**但"漏了什么"不再不可见**。

### 恢复：扫描 PDF 原文纠错与断行整理

- PDF 提取或本机 OCR 后，翻译前会调用 LLM 修复明显 OCR 错字、乱码和版面造成的错误换行。
- 清洗请求要求逐项返回并保留 `index`，模型只能标记段落边界；原始段落、修复记录和失败批次写入任务状态。
- 旧的未完成 PDF 任务恢复时会补跑该步骤；若原文发生变化，画像、术语和已有部分译文会一起失效并重新建立，避免源文与译文错位。

### 修复：Provider 路由与推理参数

- 自定义 OpenAI-compatible provider 在缺少或填写错误 Base URL 时失败闭合，不再静默回退到官方 OpenAI 地址。
- Provider 配置使用规范化地址并原子写入；快速画像和后台 worker 在调用前都会绑定当前端点。
- 已识别的推理模型可配置低/中/高推理强度；未知兼容模型保持自动模式，避免发送不支持的参数。

### 新增：扫描 PDF 自动 OCR

- 无文本层的 PDF 会自动调用本机 Tesseract；Step 1 画像使用代表页，正式翻译阶段使用全文。
- 自动探测常见安装路径和已安装语言包，不再把 `chi_sim+eng` 写死为唯一配置。
- OCR 段落会进入阶段一断点；缺少 Tesseract、语言包或页面识别失败时，具体原因写入任务警告和 `runtime_technical.log`。
